...
df <- read.dbf(here("data/raw/INEI/2010_64_RECH0.dbf"))
...
df2 <- read.dbf(here("data/raw/INEI/2010_64_RECH1.dbf")) %>% 
...
df3 <- read.dbf(here("data/raw/INEI/2010_64_RECH4.dbf")) %>% 
...
df4 <- read.dbf(here("data/raw/INEI/2010_65_RECH23.dbf")) %>% 
...
df6 <- read.dbf(here("data/raw/INEI/2010_73_REC84DV.dbf"))
...
df7 <- read.dbf(here("data/raw/INEI/REC43.dbf"))

df7[ , 168:169] <- str_split_fixed(df7$CASEID, ' ', 2)
...
df7 <- df7 %>% 
  select(V168,V169,H32Z) %>% 
  rename(
    HHID = V168,
    index = V169,
    receive.treat = H32Z) %>% 
  add_column(measure.bp = 99, measure.bsugar = 99, smoke = 99, drink =99, dentist = 99, mammogram = 99, depressed = 99, no.sleep = 99) %>% 
  mutate_all(~replace_na(.,99))
...
df_merge <- df_merge %>% 
  add_column(juntos = 99, pension65 = 99)

write.csv(df_merge, here("data/clean/2010_clean.csv"), row.names=FALSE) 
