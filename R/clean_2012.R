...
df <- read.dbf(here("data/raw/INEI/2012_64_RECH0.dbf"))
...
df2 <- read.dbf(here("data/raw/INEI/2012_64_RECH1.dbf")) %>% 
...
df3 <- read.dbf(here("data/raw/INEI/2012_64_RECH4.dbf")) %>% 
...
df4 <- read.dbf(here("data/raw/INEI/2012_65_RECH23.dbf")) %>% 
...
df6 <- read.dbf(here("data/raw/INEI/2012_73_REC84DV.dbf"))
...
df7 <- read.dbf(here("data/raw/INEI/2012_64_RECH9.dbf"))
...
df7 <- df7 %>% 
  select(HHID,SH101,SH123,SH107,SH137,SH139,SH105) %>% 
  rename(
    index = SH101,
    measure.bp = SH123,
    measure.bsugar = SH107,
    smoke = SH137,
    drink = SH139) %>% 
  add_column(dentist = 99, mammogram = 99, depressed = 99, no.sleep = 99) %>% 
  rename(
    receive.treat = SH105
  )%>% 
  mutate_all(~replace_na(.,99))
...
write.csv(df_merge, here("data/clean/2012_clean.csv"), row.names=FALSE) 
